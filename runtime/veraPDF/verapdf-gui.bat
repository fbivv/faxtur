...
"%JAVACMD%" %JAVA_OPTS%  -classpath %CLASSPATH% -XX:+IgnoreUnrecognizedVMOptions -Dfile.encoding=UTF8 -Dapp.name="VeraPDF validation GUI" -Dapp.repo="%REPO%" -Dapp.home="%BASEDIR%" -Dbasedir="%BASEDIR%" --add-exports=java.base/sun.security.pkcs=ALL-UNNAMED org.verapdf.apps.GreenfieldGuiWrapper --frameScale %SCALE_FACTOR% %CMD_LINE_ARGS%
